...
df_model <- df_model_base %>%
  mutate(
    Gambled = ifelse(as.numeric(as.character(EverGambled)) == 1, 1, 0)  # 1=gambled, 0=did not gamble
  ) %>%
  filter(
    !is.na(Gambled),
    !is.na(T_mode),
...
cat("Ever Gambled analysis sample: ", nrow(df_model), " observations\n")
...
model_interaction <- svyglm(
  Gambled ~ CAWI * Male + Cur_AgeCat + Cur_HEdQual + Cur_Ethnic6 + 
...
if (file.exists(forest_file)) {
  ...
  updated_forest <- rbind(existing_forest[existing_forest$outcome != "Ever_Gambled",], forest_data)
} else {
...
existing_main_data[["Ever_Gambled"]] <- main_effects_results
...
existing_int_data[["Ever_Gambled"]] <- interaction_results
...
ggsave("outputs_LPM/evergambled_interaction_plot.pdf", plot_evergambled, width = 6.5, height = 4.8, bg = "white")

# Show if interactive
if (interactive()) print(plot_evergambled)

cat("Results saved:\n")
cat("- Main effects: outputs_LPM/main_effects.xlsx (Ever_Gambled tab)\n")
cat("- Interactions: outputs_LPM/interaction_models.xlsx (Ever_Gambled tab)\n") 
cat("- Plot: outputs_LPM/evergambled_interaction_plot.pdf\n")