# === Sports Exercise outcome analysis ===
...
df_model <- df_model_base %>%
  mutate(
    SportsExer = ifelse(as.numeric(as.character(LeisureAct_SportsExer_q)) == 1, 1, 0)
  ) %>%
  filter(
    !is.na(SportsExer),
    !is.na(T_mode),
...
cat("Sports Exercise analysis sample: ", nrow(df_model), " observations\n")
...
model_interaction <- svyglm(
  SportsExer ~ CAWI * Male + Cur_AgeCat + Cur_HEdQual + Cur_Ethnic6 + 
...
if (file.exists(forest_file)) {
  ...
  updated_forest <- rbind(existing_forest[existing_forest$outcome != "Sports_Exercise",], forest_data)
} else {
...
existing_main_data[["Sports_Exercise"]] <- main_effects_results
...
existing_int_data[["Sports_Exercise"]] <- interaction_results
...
plot_sportsexercise <- ggplot() +
  ...
  scale_y_continuous(limits = c(-0.30, 0.30), labels = scales::percent_format()) +
...
ggsave("outputs_LPM/sportsexercise_interaction_plot.pdf", plot_sportsexercise, width = 6.5, height = 4.8, bg = "white")

# Show if interactive
if (interactive()) print(plot_sportsexercise)

cat("Results saved:\n")
cat("- Main effects: outputs_LPM/main_effects.xlsx (Sports_Exercise tab)\n")
cat("- Interactions: outputs_LPM/interaction_models.xlsx (Sports_Exercise tab)\n") 
cat("- Plot: outputs_LPM/sportsexercise_interaction_plot.pdf\n")