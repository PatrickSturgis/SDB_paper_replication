# === Currently Vapes outcome analysis ===

# Load environment and data
source("scripts/01_setup_environment.R")
source("scripts/02_load_clean_data.R")

# Outcome-specific data preparation (1=yes vapes, create binary 0/1)
df_model <- df_model_base %>%
  mutate(
    CurrentlyVapes = ifelse(as.numeric(as.character(VapeNow)) == 1, 1, 0)
  ) %>%
  filter(
    !is.na(CurrentlyVapes),
    !is.na(T_mode),
    !is.na(Cur_Sex),
    !is.na(Cur_AgeCat),
    !is.na(Cur_HEdQual),
...
cat("Currently Vapes analysis sample: ", nrow(df_model), " observations\n")
...
model_interaction <- svyglm(
  CurrentlyVapes ~ CAWI * Male + Cur_AgeCat + Cur_HEdQual + Cur_Ethnic6 + 
...
if (file.exists(forest_file)) {
  ...
  updated_forest <- rbind(existing_forest[existing_forest$outcome != "Currently_Vapes",], forest_data)
} else {
...
existing_main_data[["Currently_Vapes"]] <- main_effects_results
...
existing_int_data[["Currently_Vapes"]] <- interaction_results
...
ggsave("outputs_LPM/vaping_interaction_plot.pdf", plot_vaping, width = 6.5, height = 4.8, bg = "white")

# Show if interactive
if (interactive()) print(plot_vaping)

cat("Results saved:\n")
cat("- Main effects: outputs_LPM/main_effects.xlsx (Currently_Vapes tab)\n")
cat("- Interactions: outputs_LPM/interaction_models.xlsx (Currently_Vapes tab)\n") 
cat("- Plot: outputs_LPM/vaping_interaction_plot.pdf\n")