# === Currently Smokes outcome analysis ===

# Load environment and data
source("scripts/01_setup_environment.R")
source("scripts/02_load_clean_data.R")

# Outcome-specific data preparation (create proper binary variable)
df_model <- df_model_base %>%
  mutate(
    CurrentlySmokes = ifelse(as.numeric(as.character(SmokeNow)) == 1, 1, 0)  # 1=Yes (smokes), 2=No becomes 0
  ) %>%
  filter(
    !is.na(CurrentlySmokes),
    !is.na(T_mode),
    !is.na(Cur_Sex),
    !is.na(Cur_AgeCat),
    !is.na(Cur_HEdQual),
...
cat("Currently Smokes analysis sample: ", nrow(df_model), " observations\n")
...
model_interaction <- svyglm(
  CurrentlySmokes ~ CAWI * Male + Cur_AgeCat + Cur_HEdQual + Cur_Ethnic6 + 
...
if (file.exists(forest_file)) {
  ...
  updated_forest <- rbind(existing_forest[existing_forest$outcome != "Currently_Smokes",], forest_data)
} else {
...
existing_main_data[["Currently_Smokes"]] <- main_effects_results
...
existing_int_data[["Currently_Smokes"]] <- interaction_results
...
ggsave("outputs_LPM/currentlysmokes_interaction_plot.pdf", plot_currentlysmokes, width = 6.5, height = 4.8, bg = "white")

# Show if interactive
if (interactive()) print(plot_currentlysmokes)

cat("Results saved:\n")
cat("- Main effects: outputs_LPM/main_effects.xlsx (Currently_Smokes tab)\n")
cat("- Interactions: outputs_LPM/interaction_models.xlsx (Currently_Smokes tab)\n") 
cat("- Plot: outputs_LPM/currentlysmokes_interaction_plot.pdf\n")