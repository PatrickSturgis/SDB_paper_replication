# === Life Satisfaction outcome analysis ===
...
df_model <- df_model_base %>%
  mutate(Satis = as.numeric(as.character(Satis))) %>%
  filter(
    !is.na(Satis),
    !is.na(T_mode),
    !is.na(Cur_Sex),
  ...
  mutate(
    # Scale Satis to 0-1 range
    Satis_scaled = (Satis - min(Satis, na.rm = TRUE)) / 
      (max(Satis, na.rm = TRUE) - min(Satis, na.rm = TRUE)),
...
cat("Life Satisfaction analysis sample: ", nrow(df_model), " observations\n")
cat("Original Satis range: ", min(df_model$Satis, na.rm = TRUE), " to ", max(df_model$Satis, na.rm = TRUE), "\n")
cat("Scaled Satis range: ", min(df_model$Satis_scaled, na.rm = TRUE), " to ", max(df_model$Satis_scaled, na.rm = TRUE), "\n")
...
model_interaction <- svyglm(
  Satis_scaled ~ CAWI * Male + Cur_AgeCat + Cur_HEdQual + Cur_Ethnic6 + 
...
if (file.exists(forest_file)) {
  ...
  updated_forest <- rbind(existing_forest[existing_forest$outcome != "Life_Satisfaction",], forest_data)
} else {
...
existing_main_data[["Life_Satisfaction"]] <- main_effects_results
...
existing_int_data[["Life_Satisfaction"]] <- interaction_results
...
ggsave("outputs_LPM/lifesatisfaction_interaction_plot.pdf", plot_lifesatisfaction, width = 6.5, height = 4.8, bg = "white")

# Show if interactive
if (interactive()) print(plot_lifesatisfaction)

cat("Results saved:\n")
cat("- Main effects: outputs_LPM/main_effects.xlsx (Life_Satisfaction tab)\n")
cat("- Interactions: outputs_LPM/interaction_models.xlsx (Life_Satisfaction tab)\n") 
cat("- Plot: outputs_LPM/lifesatisfaction_interaction_plot.pdf\n")