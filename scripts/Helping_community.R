# === Help Community outcome analysis ===

# Load environment and data
source("scripts/01_setup_environment.R")
source("scripts/02_load_clean_data.R")

# Outcome-specific data preparation (create proper binary variable: 1=yes, 2=no)
df_model <- df_model_base %>%
  mutate(
    HelpsComm = ifelse(as.numeric(as.character(LeisureAct_HelpCommunity_q)) == 1, 1, 0)  # 1=yes becomes 1, 2=no becomes 0
  ) %>%
  filter(
    !is.na(HelpsComm),
    !is.na(T_mode),
...
cat("Help Community analysis sample: ", nrow(df_model), " observations\n")
...
model_interaction <- svyglm(
  HelpsComm ~ CAWI * Male + Cur_AgeCat + Cur_HEdQual + Cur_Ethnic6 + 
...
if (file.exists(forest_file)) {
  ...
  updated_forest <- rbind(existing_forest[existing_forest$outcome != "Help_Community",], forest_data)
} else {
...
existing_main_data[["Help_Community"]] <- main_effects_results
...
existing_int_data[["Help_Community"]] <- interaction_results
...
ggsave("outputs_LPM/helpcommunity_interaction_plot.pdf", plot_helpcommunity, width = 6.5, height = 4.8, bg = "white")

# Show if interactive
if (interactive()) print(plot_helpcommunity)

cat("Results saved:\n")
cat("- Main effects: outputs_LPM/main_effects.xlsx (Help_Community tab)\n")
cat("- Interactions: outputs_LPM/interaction_models.xlsx (Help_Community tab)\n") 
cat("- Plot: outputs_LPM/helpcommunity_interaction_plot.pdf\n")