...
df_model <- df_model_base %>%
  ...
  mutate(
    # Scale Happy to 0-1 range
    Happy_scaled = (Happy - min(Happy, na.rm = TRUE)) / 
      (max(Happy, na.rm = TRUE) - min(Happy, na.rm = TRUE)),
...
cat("Happy analysis sample: ", nrow(df_model), " observations\n")
cat("Original Happy range: ", min(df_model$Happy, na.rm = TRUE), " to ", max(df_model$Happy, na.rm = TRUE), "\n")
cat("Scaled Happy range: ", min(df_model$Happy_scaled, na.rm = TRUE), " to ", max(df_model$Happy_scaled, na.rm = TRUE), "\n")
...
model_interaction <- svyglm(
  Happy_scaled ~ CAWI * Male + Cur_AgeCat + Cur_HEdQual + Cur_Ethnic6 + 
...
if (file.exists(forest_file)) {
  ...
  updated_forest <- rbind(existing_forest[existing_forest$outcome != "Happy",], forest_data)
} else {
...
existing_main_data[["Happy"]] <- main_effects_results
...
existing_int_data[["Happy"]] <- interaction_results
...
ggsave("outputs_LPM/happy_interaction_plot.pdf", plot_happy, width = 6.5, height = 4.8, bg = "white")

# Show if interactive
if (interactive()) print(plot_happy)

cat("Results saved:\n")
cat("- Main effects: outputs_LPM/main_effects.xlsx (Happy tab)\n")
cat("- Interactions: outputs_LPM/interaction_models.xlsx (Happy tab)\n") 
cat("- Plot: outputs_LPM/happy_interaction_plot.pdf\n")