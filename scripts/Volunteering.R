# === Volunteering outcome analysis ===

# Load environment and data
source("scripts/01_setup_environment.R")
source("scripts/02_load_clean_data.R")

# Outcome-specific data preparation (reverse coding: 0=did volunteer, 1=did not volunteer)
df_model <- df_model_base %>%
  mutate(
    Volunteers = ifelse(as.numeric(as.character(VolUnPd07)) == 0, 1, 0)  # Reverse: 0 becomes 1 (volunteers)
  ) %>%
  filter(
    !is.na(Volunteers),
    !is.na(T_mode),
...
cat("Volunteering analysis sample: ", nrow(df_model), " observations\n")
...
model_interaction <- svyglm(
  Volunteers ~ CAWI * Male + Cur_AgeCat + Cur_HEdQual + Cur_Ethnic6 + 
...
if (file.exists(forest_file)) {
  ...
  updated_forest <- rbind(existing_forest[existing_forest$outcome != "Volunteering",], forest_data)
} else {
...
existing_main_data[["Volunteering"]] <- main_effects_results
...
existing_int_data[["Volunteering"]] <- interaction_results
...
plot_volunteering <- ggplot() +
  ...
  # P-value annotations
  annotate("text", x = 1, y = 0.19, 
           label = main_p_label, size = 3, fontface = "italic") +
  annotate("text", x = 2, y = 0.19, 
...
ggsave("outputs_LPM/volunteering_interaction_plot.pdf", plot_volunteering, width = 6.5, height = 4.8, bg = "white")

# Show if interactive
if (interactive()) print(plot_volunteering)

cat("Results saved:\n")
cat("- Main effects: outputs_LPM/main_effects.xlsx (Volunteering tab)\n")
cat("- Interactions: outputs_LPM/interaction_models.xlsx (Volunteering tab)\n") 
cat("- Plot: outputs_LPM/volunteering_interaction_plot.pdf\n")