# === At Risk Gambling outcome analysis ===

# Load environment and data
source("scripts/01_setup_environment.R")
source("scripts/02_load_clean_data.R")

# Outcome-specific data preparation (derive from PGSI_total: 0=0, >0=1)
df_model <- df_model_base %>%
  mutate(
    AtRiskGambling = ifelse(!is.na(PGSI_total) & PGSI_total > 0, 1, 0)
  ) %>%
  filter(
    !is.na(PGSI_total),
    !is.na(T_mode),
...
cat("At Risk Gambling analysis sample: ", nrow(df_model), " observations\n")
...
model_interaction <- svyglm(
  AtRiskGambling ~ CAWI * Male + Cur_AgeCat + Cur_HEdQual + Cur_Ethnic6 + 
...
if (file.exists(forest_file)) {
  ...
  updated_forest <- rbind(existing_forest[existing_forest$outcome != "At_Risk_Gambling",], forest_data)
} else {
...
existing_main_data[["At_Risk_Gambling"]] <- main_effects_results
...
existing_int_data[["At_Risk_Gambling"]] <- interaction_results
...
ggsave("outputs_LPM/atriskgambling_interaction_plot.pdf", plot_atriskgambling, width = 6.5, height = 4.8, bg = "white")

# Show if interactive
if (interactive()) print(plot_atriskgambling)

cat("Results saved:\n")
cat("- Main effects: outputs_LPM/main_effects.xlsx (At_Risk_Gambling tab)\n")
cat("- Interactions: outputs_LPM/interaction_models.xlsx (At_Risk_Gambling tab)\n") 
cat("- Plot: outputs_LPM/atriskgambling_interaction_plot.pdf\n")