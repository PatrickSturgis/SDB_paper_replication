# === Cannabis outcome analysis ===

# Load environment and data
source("scripts/01_setup_environment.R")
source("scripts/02_load_clean_data.R")

# Outcome-specific data preparation (1=yes, 2=no, recode to binary)
df_model <- df_model_base %>%
  mutate(
    EverCannabis = ifelse(as.numeric(as.character(CanEver)) == 1, 1, 0)  # 1=yes (used cannabis), 2=no becomes 0
  ) %>%
  filter(
    !is.na(EverCannabis),
    !is.na(T_mode),
...
cat("Cannabis analysis sample: ", nrow(df_model), " observations\n")
...
model_interaction <- svyglm(
  EverCannabis ~ CAWI * Male + Cur_AgeCat + Cur_HEdQual + Cur_Ethnic6 + 
...
if (file.exists(forest_file)) {
  ...
  updated_forest <- rbind(existing_forest[existing_forest$outcome != "Cannabis",], forest_data)
} else {
...
existing_main_data[["Cannabis"]] <- main_effects_results
...
existing_int_data[["Cannabis"]] <- interaction_results
...
ggsave("outputs_LPM/cannabis_interaction_plot.pdf", plot_cannabis, width = 6.5, height = 4.8, bg = "white")

# Show if interactive
if (interactive()) print(plot_cannabis)

cat("Results saved:\n")
cat("- Main effects: outputs_LPM/main_effects.xlsx (Cannabis tab)\n")
cat("- Interactions: outputs_LPM/interaction_models.xlsx (Cannabis tab)\n") 
cat("- Plot: outputs_LPM/cannabis_interaction_plot.pdf\n")