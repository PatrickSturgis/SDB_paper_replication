# === General Health outcome analysis ===
...
df_model <- df_model_base %>%
  mutate(GnrlHealth = as.numeric(as.character(GnrlHealth))) %>%
  filter(
    !is.na(GnrlHealth),
    !is.na(T_mode),
    !is.na(Cur_Sex),
  ...
  mutate(
    # Reverse code GnrlHealth so higher scores = better health
    GnrlHealth_reversed = (max(GnrlHealth, na.rm = TRUE) + min(GnrlHealth, na.rm = TRUE)) - GnrlHealth,
    # Scale reversed GnrlHealth to 0-1 range
    GnrlHealth_scaled = (GnrlHealth_reversed - min(GnrlHealth_reversed, na.rm = TRUE)) / 
      (max(GnrlHealth_reversed, na.rm = TRUE) - min(GnrlHealth_reversed, na.rm = TRUE)),
...
cat("General Health analysis sample: ", nrow(df_model), " observations\n")
cat("Original GnrlHealth range: ", min(df_model$GnrlHealth, na.rm = TRUE), " to ", max(df_model$GnrlHealth, na.rm = TRUE), "\n")
cat("Reversed GnrlHealth range: ", min(df_model$GnrlHealth_reversed, na.rm = TRUE), " to ", max(df_model$GnrlHealth_reversed, na.rm = TRUE), " (higher = better health)\n")
cat("Scaled GnrlHealth range: ", min(df_model$GnrlHealth_scaled, na.rm = TRUE), " to ", max(df_model$GnrlHealth_scaled, na.rm = TRUE), "\n")
...
model_interaction <- svyglm(
  GnrlHealth_scaled ~ CAWI * Male + Cur_AgeCat + Cur_HEdQual + Cur_Ethnic6 + 
...
if (file.exists(forest_file)) {
  ...
  updated_forest <- rbind(existing_forest[existing_forest$outcome != "General_Health",], forest_data)
} else {
...
existing_main_data[["General_Health"]] <- main_effects_results
...
existing_int_data[["General_Health"]] <- interaction_results
...
plot_generalhealth <- ggplot() +
  ...
  scale_y_continuous(limits = c(-0.20, 0.20), labels = scales::percent_format()) +
...
ggsave("outputs_LPM/generalhealth_interaction_plot.pdf", plot_generalhealth, width = 6.5, height = 4.8, bg = "white")

# Show if interactive
if (interactive()) print(plot_generalhealth)

cat("Results saved:\n")
cat("- Main effects: outputs_LPM/main_effects.xlsx (General_Health tab)\n")
cat("- Interactions: outputs_LPM/interaction_models.xlsx (General_Health tab)\n") 
cat("- Plot: outputs_LPM/generalhealth_interaction_plot.pdf\n")